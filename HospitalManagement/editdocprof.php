...
<?php
session_start(); 
 if (isset($_SESSION['doctor_id'])==false)
{
...
<li>
<a href="login_form.php" class="hvr-bounce-to-right">Log in</a>
</li>
...
<a href="doctor_personal_prof.php"><li class="hvr-sweep-to-bottom "><h2><i>View Profile</i></h2></a>
</li>
...
session_start();
	if(isset($_SESSION['doctor_id'])) {
        ...
        $doc_id= $_SESSION['doctor_id'];
		 ...
		 $sql = "SELECT * FROM `doc_img_up` WHERE `doctor_id` = '$doc_id'";
		 $result=mysql_query($sql);
    while ($row=mysql_fetch_array($result)) {
		
		$doctor_name=$row['doctor_name'];
		$doc_email=$row['doc_email'];
		 $doctor_catagory=$row['doctor_catagory'];
		 $Nationality=$row['Nationality'];
		 $doctor_visit=$row['doctor_visit'];
		  $doctor_degree=$row['doctor_degree'];
		   $doctor_address=$row['doctor_address'] ;
		  $doctor_post=$row['doctor_post'];
...
<br><br>
								Doctor Name:<input type="text"  name="doctor_name" style="padding:5px 15px" value="<?php echo $doctor_name;?>"  required><br><br>
								Doctor Email:<input type="email"  name="doctor_email" style="padding:5px 15px" value="<?php echo $doc_email;?>"  required><br><br>
							...
							Degree:<input type="text"  name="doctor_degree" style="padding:5px 15px" value="<?php echo $doctor_degree;?>"  required ><br><br>
							Nationality:<input type="text"  name="Nationality" style="padding:5px 15px"value="<?php echo $Nationality;?>"  required><br><br>
							Address:<input type="text"  name="doctor_address" style="padding:5px 15px" value="<?php echo $doctor_address;?>" required><br><br>
							Post:<input type="text"  name="doctor_post" style="padding:5px 15px" value="<?php echo $doctor_post;?>" required><br><br>
							Job type:<input type="text"  name="job_type" style="padding:5px 15px" value="<?php echo $job_type;?>" required><br><br>
								Visiting Hour:<input type="text"  name="doctor_visit" style="padding:5px 15px" value="<?php echo $doctor_visit;?>" required><br><br>
...
//$sqlll= "UPDATE `doc_img_up`  WHERE `doctor_id` = '$doc_id' SET`doctor_id`='$doc_id', `doctor_name`='$doctor_name', `doc_email`='$doctor_email', `doctor_address`='', `Nationality`='$Nationality',  `doctor_degree`='$doctor_degree', `doctor_post`='$doctor_post', `job_type`='$job_type', `doctor_visit`='$doctor_visit', `doctor_img`='$doctor_img'";   
//mysql_query($sqlll);
...
$sqlll= "UPDATE `doc_img_up`  SET`doctor_id`='$doc_id', `doctor_name`='$doctor_name', `doc_email`='$doctor_email', `doctor_address`='$doctor_address', `Nationality`='$Nationality',  `doctor_degree`='$doctor_degree', `doctor_post`='$doctor_post', `job_type`='$job_type', `doctor_visit`='$doctor_visit',`doctor_img`='$doctor_img'  WHERE `doctor_id` = '$doc_id'";   
mysql_query($sqlll);
